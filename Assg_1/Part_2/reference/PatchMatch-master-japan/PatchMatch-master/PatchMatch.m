...
for ii = 1:tsz(1)
  for jj = 1:tsz(2)
                    % ofs_ini is a 9x9 patch matrix initially
    ofs_ini = targetImg_NaN(w+ii-w:w+ii+w,w+jj-w:w+jj+w)...
          - sourceImg(NNF(ii,jj,1)-w:NNF(ii,jj,1)+w,NNF(ii,jj,2)-w:NNF(ii,jj,2)+w);

    ofs_ini = ofs_ini(~isnan(ofs_ini(:))); % removes nan numers and converts patch matrix (ideally 81 elements) into a vector
    offsets(ii,jj) = sum(ofs_ini.^2)/length(ofs_ini); % sum of squared vector element entries divided by no. of elements
...
for ii = ii_seq
  for jj = jj_seq
    ...
    if is_odd %odd
        % offset(ii,jj) contains sum of squared differences for each pixel between
        % targetImg_NaN and sourceImg 
